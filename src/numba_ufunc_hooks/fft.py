"""
Expose numpy's FFT functionality to numba by adding LLVM hooks that
...
from numba_ufunc_hooks.ufunc_minimal import MinimalPyUFuncObject
...
# to be cached.
FFT_UFUNC_TABLE_SYMBOL = "_numba_ufunc_hooks_pocketfft_ufunc_table"
...
def _install_ufunc_table(pfu):
    ...
    for u, ufunc_name in enumerate(FFT_UFUNCS):
        ...
        for t in range(ufunc.ntypes):
            arg_type_ids = tuple(
                ufunc.types[t * ufunc.nargs + a] for a in range(ufunc.nargs)
            )
...
def intrinsic_fft_factory(ufunc_name):
    ...
    def _invoke_fft_ufunc(typingctx, a, factor, out):
        ...
        try:
            ...
            raise KeyError(
                f"{key} ufunc type variant not registered in the fft ufunc table"
            )
        ...
        def codegen(context, builder, signature, args):
            ...
            builder.store(
                builder.bitcast(float_ptr, voidptr_t), builder.gep(args_ptr, [ONE])
            )
            builder.store(
                builder.bitcast(out_struct.data, voidptr_t),
                builder.gep(args_ptr, [TWO]),
            )
            ...
            if bool(FFT_UFUNC_TABLE[ufunc_id].data):
                ...
                data_address = builder.load(
                    builder.bitcast(data_field, ir.PointerType(i64_t))
                )
                data_address_ptr = builder.inttoptr(data_address, ir.PointerType(i64_t))
                data_int = builder.load(
                    builder.gep(data_address_ptr, [TYPE_VARIANT_OFFSET])
                )
